...
- Subagent reads one or more relevant SKILL.md files
- Answer references `core/iq-tokens.css` or the skills, not fabricated values
- Color choice is KGiQ parent gold or steel blue, NOT iQ Publish indigo or iQ Booking teal (stray product accents would indicate the locker didn't surface correctly)
...
- [ ] `KGiQ-LLC/iq-design-locker/` directory structure exists
- [ ] `core/iq-tokens.css` has no misleading comments, uses gold as the neutral default
- [ ] Three `accents/iq-accent-*.css` files exist with correct values
- [ ] `parent-layer/iq-site-tokens.css` is clean (no imports, no product accent)
- [ ] `docs/` has authoring spec, operational brief, components guide, panel history
- [ ] `LINEAGE.md` documents full provenance, drift reconciliation, consumer delta, completeness check, and spec verification
- [ ] 7 active + 1 dormant SKILL.md files with valid Claude Code native frontmatter
- [ ] `MANIFEST.md`, `README.md`, `scripts/vendor-into.sh` all exist and are correct
- [ ] `KGiQ-LLC/CLAUDE.md` has a Lockers section entry
- [ ] One successful empirical spawn discoverability test
- [ ] `iq-locker/docs/lockers-index.md` status flipped to `validated`
