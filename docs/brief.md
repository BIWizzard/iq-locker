...
|----|-----------|--------|-----------|
| A1 | Subagents in Claude Code still don't auto-inherit parent-session skill registrations. MANIFEST-read protocol remains necessary. | **validated (direct)** | 2026-04-10 via T3 Hearst POC + Claude Flow audit + Claude Code native audit (indirect). **2026-04-12 via direct empirical spawn discoverability test**: blank-slate general-purpose subagent in KGiQ-LLC found canonical source of truth in 3 reads via `CLAUDE.md` → `MANIFEST.md` → `iq-brand-system.md`, explicitly read SKILL.md binding rules before acting, refused to use `--iq-accent` for interactive elements. This is the gold standard A1 validation. *Nuanced:* subagents DO inherit project-level skills from disk (description layer), but NOT the parent's conversation-level discovery. MANIFEST remains necessary for curation (active/dormant), trust boundaries, and promotion discipline — not for physical file availability. |
...
| A3 | Skill locker pattern generalizes cleanly from vendored-upstream (Fabric) to self-authored content (IQ brand). | **validated** | 2026-04-10 → 2026-04-12 across Phase 2 (Waves 1-4 build, 2026-04-12 consolidation detour, Wave 5-6 wiring). Locker absorbed 3 sessions of construction + major mid-flight consolidation without structural issues. Three-layer content model (core + accents + parent-layer) + MANIFEST curation held up under expansion to 4 products. Empirical spawn discoverability test passed first try. See `docs/sessions/2026-04-{10,11,12}*.md` + `KGiQ-LLC/iq-design-locker/LINEAGE.md` §1-8. |
...
| A11 | The first-instance IQ Design System locker lives at `KGiQ-LLC/iq-design-locker/` and BECOMES the canonical source of truth for the IQ design system. It is not vendoring from an upstream — it IS the upstream. Consumer projects (iq-publish, booking-module, portfolio-front-end, future) will vendor FROM it. | **validated** | 2026-04-10 decision + 2026-04-12 empirical validation via spawn discoverability test: blank-slate subagent in KGiQ-LLC correctly discovered the locker and identified it as the canonical source with zero priming. Also: iq-circle was seeded as a new consumer and vendored from the locker at inception 2026-04-12. The canonical-source claim now has both structural (build passed) and empirical (spawn test) evidence. Phase E will reconcile remaining consumers. |
| A12 | Every locker build includes a backwards-looking provenance audit (`LINEAGE.md`) that records per-file source, byte-diffs against origin copies, and drift reconciliation decisions. This is a structural quality gate, not optional. | **validated** | 2026-04-11 during Waves 1-4 build (LINEAGE caught factual errors in cycle 1 code review that would have silently propagated). 2026-04-12 during consolidation detour (§7 addition worked as expected; §7.9 path resolution table + MANIFEST §6 routing solved discoverability). 2026-04-12 Wave 5-6 (§8 addition captured spawn test audit trail + 3 content gaps). LINEAGE continues to be the authoritative "what happened to this locker" record across 4 sessions. |
